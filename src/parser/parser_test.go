...
func TestNewParserWindow(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department) AS running_total
	FROM employees;
`)
...
func TestNewParserWindow2(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department, another_id) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.PartitionBy[1].Value.(*ColumnSpecification).ColumnName.Value != "another_id" {
		t.Fatalf("expected another_id, got %s", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.PartitionBy[1].Value.(*ColumnSpecification).ColumnName.Value)
	}
...
func TestNewParserWindow4(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC  ) AS running_total
	FROM employees;
...
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != ROWS_UNBOUNDED_PRECEDING_CURRENT_ROW {
		t.Fatalf("expected ROWS_UNBOUNDED_PRECEDING_CURRENT_ROW, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}
...
func TestNewParserWindow6(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN UNBOUNDED PRECEDING AND 1 FOLLOWING ) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != ROWS_UNBOUNDED_PRECEDING_LITERAL_FOLLOWING {
...
func TestNewParserWindow9(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING ) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != ROWS_CURRENT_ROW_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected ROWS_CURRENT_ROW_UNBOUNDED_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

}

func TestNewParserWindow10(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN 1 PRECEDING AND 6 FOLLOWING ) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != ROWS_LITERAL_PRECEDING_LITERAL_FOLLOWING {
		t.Fatalf("expected ROWS_LITERAL_PRECEDING_LITERAL_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}
...
func TestNewParserWindow11(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN 5 PRECEDING AND CURRENT ROW ) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != ROWS_LITERAL_PRECEDING_CURRENT_ROW {
		t.Fatalf("expected ROWS_LITERAL_PRECEDING_CURRENT_ROW, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Lower.Value != uint64(5) {
		t.Fatalf("expected 5, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Lower.Value)
	}

}

func TestNewParserWindow12(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN 3 PRECEDING AND UNBOUNDED FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != ROWS_LITERAL_PRECEDING_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected ROWS_LITERAL_PRECEDING_UNBOUNDED_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Lower.Value != uint64(3) {
		t.Fatalf("expected 3, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Lower.Value)
	}

}

func TestNewParserWindow13(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN UNBOUNDED PRECEDING AND 4 FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.FrameType != WINDOW_FRAME_RANGE {
		t.Fatalf("expected WINDOW_FRAME_RANGE, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.FrameType)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_UNBOUNDED_PRECEDING_LITERAL_FOLLOWING {
		t.Fatalf("expected RANGE_UNBOUNDED_PRECEDING_LITERAL_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Upper.Value != uint64(4) {
		t.Fatalf("expected 4, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Upper.Value)
	}
}

func TestNewParserWindow14(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_UNBOUNDED_PRECEDING_CURRENT_ROW {
		t.Fatalf("expected RANGE_UNBOUNDED_PRECEDING_CURRENT_ROW, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

}

func TestNewParserWindow15(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_UNBOUNDED_PRECEDING_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected RANGE_UNBOUNDED_PRECEDING_UNBOUNDED_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

}

func TestNewParserWindow16(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_CURRENT_ROW_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected RANGE_CURRENT_ROW_UNBOUNDED_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

}

func TestNewParserWindow17(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN CURRENT ROW AND 2 FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_CURRENT_ROW_LITERAL_FOLLOWING {
		t.Fatalf("expected RANGE_CURRENT_ROW_LITERAL_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Upper.Value != uint64(2) {
		t.Fatalf("expected 2, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Upper.Value)
	}

}

func TestNewParserWindow18(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN 1 PRECEDING AND 2 FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_LITERAL_PRECEDING_LITERAL_FOLLOWING {
		t.Fatalf("expected RANGE_LITERAL_PRECEDING_LITERAL_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}
...
func TestNewParserWindow19(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN 1 PRECEDING AND CURRENT ROW) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_LITERAL_PRECEDING_CURRENT_ROW {
		t.Fatalf("expected RANGE_LITERAL_PRECEDING_CURRENT_ROW, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}
...
func TestNewParserWindow20(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC RANGE BETWEEN 7 PRECEDING AND UNBOUNDED FOLLOWING) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type != RANGE_LITERAL_PRECEDING_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected RANGE_LITERAL_PRECEDING_UNBOUNDED_FOLLOWING, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Type)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Lower.Value != uint64(7) {
		t.Fatalf("expected 7, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Boundary.Lower.Value)
	}
...
func TestNewParserDistFunc3(t *testing.T) {
	statement := []byte(`
...
`)
	...
	if selectStmt.SelectList.Expressions[0].Value.(*PercentileContFunc) == nil {
		t.Fatalf("expected PercentileContFunc, got %T", selectStmt.SelectList.Expressions[0].Value.(*PercentileContFunc))
	}

	if selectStmt.SelectList.Expressions[0].Value.(*PercentileContFunc).Expr.(*Literal).Value != float64(0.5) {
		t.Fatalf("expected 0.5, got %f", selectStmt.SelectList.Expressions[0].Value.(*PercentileContFunc).Expr.(*Literal).Value)
	}
