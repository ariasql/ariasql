...
func TestNewParserWindow9(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN 3 PRECEDING AND CURRENT ROW ) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.Type != WINDOW_FRAME_BOUND_N_PRECEDING_CURRENT_ROW {
		t.Fatalf("expected ROWS, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.Type)
	}

	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.N.Value != uint64(3) {
		t.Fatalf("expected 3, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.N)
	}
...
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.Type != WINDOW_FRAME_BOUND_N_PRECEDING_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected ROWS, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.Type)
	}
...
func TestNewParserWindow11(t *testing.T) {
	statement := []byte(`
	SELECT employee_id, salary,
		   SUM(salary) OVER (PARTITION BY department ORDER BY salary DESC ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING ) AS running_total
	FROM employees;
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.N.Value != uint64(2) {
		t.Fatalf("expected 3, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.N)
	}
...
`)
	...
	if selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.Type != WINDOW_FRAME_BOUND_CURRENT_ROW_UNBOUNDED_FOLLOWING {
		t.Fatalf("expected ROWS, got %d", selectStmt.SelectList.Expressions[2].Value.(*WindowFunc).Spec.Frame.Bound.Type)
	}
